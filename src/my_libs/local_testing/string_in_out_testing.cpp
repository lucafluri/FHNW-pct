 ...
 */
void prepare_ide(const string &problem_no, int file_no) {
    if (file_no == -1) {
        cout << "Which file-set? ";
        cin >> file_no;
    }

    sub_path = "in_out_exp_files/" + problem_no + "/";
    ...
    if (!freopen(in_name.c_str(), "r", stdin)) {
        throw runtime_error("Problems reading from file " + in_name);
    }
...
void execute_tests() {
    ...
    *stdout = *fp_out;                          // NOLINT(misc-non-copyable-objects)
     ...
     * Doesn't yet work ?!   (But this is not necessary for this project)
     */
//    fclose(stdin);
//    FILE  *fp_in = fdopen(old_stdin, "r");  // Open old stdout as a stream
...
//        cout << "\n----------------" << endl;
        ...
        if (error_count > 0) {
            write_colored_text(
                    "You have " + to_string(error_count) + " wrong line" + (error_count == 1 ? "" : "s") + "!\n",
                    magenta);
        } else {
            write_colored_text("Yes, no errors!\n", green);
        }
    }
